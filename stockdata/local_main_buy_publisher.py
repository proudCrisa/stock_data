...
def _validate_announcement_capture(directory, symbol, source_files, observed, *, observation_end=None,
    ...
    _validate_event_attachments(announcements, evidence or {"files": []}, events,
        ...
        known_baseline=hashlib.sha256(raw).hexdigest() == _REVIEWED_ANNOUNCEMENT_HASHES.get(symbol))
...
def prepare_dynamic_reference_inputs(*, evidence_dir, liquidity_product, asof,
                                     global_snapshot, candidate_instrument_authority,
    ...
    symbols = sorted(item["symbol"] for item in verified["profile"]["candidates"])
    if index.get("schema_version") != "stockdata-main-etf-reviewed-facts/1" \
            or index.get("asof") != asof or set(index.get("instruments", {})) != set(symbols):
        raise ValueError("dynamic reviewed ETF facts differ exact candidate profile")
    observed = datetime.now(timezone.utc).isoformat()
    observation_end = _day(observation_end or _timestamp(observed).astimezone(
        timezone(timedelta(hours=8))).date().isoformat())
    observed_day = _timestamp(observed).astimezone(
        timezone(timedelta(hours=8))).date().isoformat()
    if not asof <= observation_end <= observed_day:
        raise ValueError("dynamic reference observation window is invalid")
    scopes = {**ETF_RULE_SCOPES, **verified["scopes"]}
    rows = {component: {} for component in (
        "trading_calendar", "instrument_status", "market_rules", "universe",
        "corporate_actions")}
    evidence = {component: {
        "schema_version": "stockdata-local-reference-source-evidence/1",
        "assessment": "publisher-reviewed-dynamic-candidate",
        "files": [], "additional_instruments": {},
        "candidate_instrument_authority_sha256": verified["authority_sha256"],
    } for component in rows}
    history_by_symbol = {symbol: [entry.split("@")[1]
                         for entry in liquidity_product["panel"]
                         if entry.startswith(symbol + "@")] for symbol in symbols}
    for symbol in symbols:
        facts = index["instruments"][symbol]
        if facts["corporate_actions_coverage"] != {
                "start_date": "2025-07-11", "end_date": asof, "complete": True}:
            raise ValueError("dynamic ETF corporate-action coverage is incomplete")
        _validate_reviewed_events(facts, symbol)
        bound = {component: _evidence(
            directory, index, facts["source_files"][component],
            facts["assessments"][component]) for component in rows}
        for item in bound.values():
            _validate_retained_times(item, observed)
        status_file = f"{symbol}-status-ca.json"
        captures = json.loads((directory / status_file).read_bytes())
        state = _validate_status_capture(captures, symbol, asof, observed)
        if status_file not in facts["source_files"]["instrument_status"]:
            raise ValueError("dynamic ETF status facts are not receipt-bound")
        _validate_announcement_capture(
            directory, symbol, facts["source_files"]["corporate_actions"], observed,
            observation_end=observation_end, evidence=bound["corporate_actions"],
            events=facts["events"],
            reviewed_non_action_urls=facts.get("reviewed_non_action_urls"))
        universe = _validate_universe(
            directory, symbol, facts["source_files"]["universe"],
            candidate_profile=verified["profile"])
        calendar_name = next((name for name in facts["source_files"]["trading_calendar"]
                              if name.endswith(".json") and json.loads(
                                  (directory / name).read_bytes()).get("request", {}).get(
                                      "method") == "query_trade_dates"), None)
        if calendar_name is None:
            raise ValueError("dynamic ETF calendar capture is missing")
        calendar = json.loads((directory / calendar_name).read_bytes())
        days = [row[0] for row in calendar["response"]["rows"] if row[1] == "1"]
        history = history_by_symbol[symbol]
        if calendar["response"].get("error_code") != "0" or days[:-1] != history \
                or history[-1] != asof or calendar["request"] != {
                    "method": "query_trade_dates", "start_date": history[0],
                    "end_date": days[-1]}:
            raise ValueError("dynamic ETF calendar does not cover exact amount sessions")
        fees_name = next((name for name in facts["source_files"]["market_rules"]
                          if name.endswith(".json") and json.loads(
                              (directory / name).read_bytes()).get("policy_version") ==
                          "broker-fee-v1"), None)
        if fees_name is None:
            raise ValueError("dynamic ETF fee policy source is missing")
        fees = json.loads((directory / fees_name).read_bytes())
        entry = f"{symbol}@{asof}"
        for day, next_day in zip(days, days[1:]):
            rows["trading_calendar"][f"{symbol}@{day}"] = {
                "is_trading_day": True,
        ...
        rule = {
            ...
            "policy_id": f"{symbol}-current-local-v1",
            "source": "local-reviewed-official-facts/1",
            "source_sha256": "", "security_type": "ETF", "board": "ETF",
            "instrument_id": symbol, **scopes[symbol], "effective_until": asof,
            "listing_age_min": 0, "listing_age_max": None, "is_st": False,
            "reject_suspended": True, "reject_zero_volume": True,
            "price_limit_reference": "RECORD_OR_PREVIOUS_CLOSE",
            "price_rounding": "HALF_UP", "locked_limit_order_policy": "REJECT_SIDE",
            "commission_rate": fees["commission_rate"],
            "minimum_commission": fees["min_commission_cny"],
            "transfer_fee_rate": 0., "stamp_duty_sell_rate": fees["etf_stamp_tax"],
            "slippage_model": "OPEN_BPS", "slippage_bps": 0.,
            "slippage_bounds": "BAR_AND_PRICE_LIMITS", "time_in_force": "DAY",
            "cancel_unfilled_at_close": True,
        }
        events = [{**item, "announcement_at": observed} for item in facts["events"]]
        event_hashes = {item["sha256"] for item in bound["corporate_actions"]["files"]}
        if any(item["event_id"] not in event_hashes for item in events):
            raise ValueError("dynamic ETF corporate action lacks retained official source")
        rows["instrument_status"][entry] = {
            "is_st": False, "is_suspended": False, "listing_status": "listed"}
        rows["universe"][entry] = {
            "is_member": True, "universe_id": universe["source_sha256"]}
        rows["corporate_actions"][entry] = {"events": events}
        rows["market_rules"][entry] = rule
        bound["instrument_status"]["raw_baostock_isST"] = state["isST"]
        for component in rows:
            evidence[component]["additional_instruments"][symbol] = bound[component]
    for rule in rows["market_rules"].values():
        rule["source_sha256"] = _hash(evidence["market_rules"])
    references = {component: _reference(component, values, evidence[component], observed)
                  for component, values in rows.items()}
    global_names = index.get("source_files", {}).get("global_signals")
    if not isinstance(global_names, list) or not global_names:
        raise ValueError("dynamic global snapshot source evidence is missing")
    global_evidence = _evidence(
        directory, index, global_names,
        index.get("assessments", {}).get("global_signals"))
    _validate_retained_times(global_evidence, observed)
    original = json.loads((directory / global_names[0]).read_bytes())
    for name, quote in global_snapshot["quotes"].items():
        source = original.get(name)
        if source is None or quote != {
                "price": source["price"], "chg_pct": source["chg_pct"],
                "chg_20d": source["chg_20d"], "date": source["last_date"]}:
            raise ValueError("dynamic global snapshot differs retained source facts")
    global_evidence["candidate_instrument_authority_sha256"] = \
        verified["authority_sha256"]
    global_inputs = build_global_signals_authority_inputs(
        global_snapshot, panel=[f"{symbol}@{asof}" for symbol in symbols],
        available_at=observed, source_evidence=global_evidence)
    return references, global_inputs


def publish_main_buy_supplement(*, evidence_dir, global_snapshot_file, publisher_dir,
                               registry_sha256, provider_manifest_sha256, output_dir,
                               liquidity_capture_file=None, additional_evidence_dir=None,
                               candidate_instrument_authority_file=None,
    ...
    if candidate_instrument_authority_file is not None:
        candidate_authority = load_candidate_instrument_authority(
            ...
            decision_cutoff=datetime.now(timezone.utc).isoformat(), registry=registry)
        symbols = sorted(item["symbol"] for item in
                         candidate_authority["candidate_profile"]["candidates"])
        if additional_evidence_dir is None:
            raise ValueError("dynamic candidate supplement requires additional evidence")
        captures = [
            json.loads((Path(additional_evidence_dir) / f"{symbol}-amount.json").read_bytes())
            for symbol in symbols]
    else:
        if liquidity_capture_file is None:
            raise ValueError("fixed supplement requires a liquidity capture")
        ...
        if additional_evidence_dir is not None:
            ...
            captures.extend(json.loads(
                (Path(additional_evidence_dir) / f"{symbol}-amount.json").read_bytes())
                for symbol in symbols if symbol != SYMBOL)
    capture = captures[0]
    capture_asof = capture["request"]["end_date"]
    if asof is not None and _day(asof) != capture_asof:
        raise ValueError("requested asof differs from native amount capture")
    asof = capture_asof
    if candidate_authority is not None and \
            candidate_authority["candidate_profile"]["asof"] != asof:
        raise ValueError("candidate authority asof differs native amount capture")
    ...
    global_snapshot = json.loads(Path(global_snapshot_file).read_bytes())
    if candidate_authority is not None:
        references, global_inputs = prepare_dynamic_reference_inputs(
            evidence_dir=additional_evidence_dir, liquidity_product=product,
            asof=asof, global_snapshot=global_snapshot,
            candidate_instrument_authority=candidate_authority,
            observation_end=observation_end)
    else:
        references, global_inputs = prepare_reference_inputs(
            evidence_dir=evidence_dir, liquidity_product=product,
            asof=asof, global_snapshot=global_snapshot,
            observation_end=observation_end)
    if additional_evidence_dir is not None and candidate_authority is None:
        references, global_inputs = extend_reference_inputs(references, global_inputs,
            evidence_dir=additional_evidence_dir,
            symbols=sorted(set(symbols) | {SYMBOL}), asof=asof,
            observation_end=observation_end,
            candidate_instrument_authority=candidate_authority)
    def sign(inputs):
