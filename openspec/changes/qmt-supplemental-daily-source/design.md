...
### D4c: 快照主路径 + fulldata 备路径 + 单因子全量刷新
...
- **单因子版本全量刷新**:前复权在分红除权后会重述全部历史,滚动窗口会造成
  新旧因子混排。因此每次同步把通道返回的**整段历史**整体 upsert,并删除
  通道返回范围之外的更早日行;重述后转为非正价的日期(累计除权产物,
  受 Cache 正价不变量约束不入库)作为已观测证据参与覆盖声明,并清除其
  旧因子残留行。库内 qmt 序列恒为单一复权因子版本。
- **覆盖声明防夸大**:既有覆盖与本次验证区间不相交且夹缝含交易日时
  (宕机超窗),拒绝 MIN/MAX 合并声明,只保留已验证段。
